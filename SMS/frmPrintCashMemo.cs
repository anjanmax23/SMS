...
namespace SMS
{
    public partial class frmPrintCashMemo : Form
    {
        string ConnString;
        ...
        private void frmPrintCashMemo_Load(object sender, EventArgs e)
        {
            this.Left = 250;
            this.Top = 250;
        }

        private void ReadInvoiceHead(int OrderID)
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("GetCompanyDetailsForCashMemoPrintByOrderID", con))
                    {
...
//                                                FieldValue = "" + dr["TaxType"];
        ...
        private int UpdateTokenPrintStatus(int OrderID)
        {
            try
            {
                using (SqlConnection con = new SqlConnection(ConnString))
                {
                    using (SqlCommand cmd = new SqlCommand("UpdateCashMemoStatusByOrderID", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@OrderID", OrderID);
                        cmd.CommandTimeout = 0;
        ...
        private bool CheckInvoicePrintStatus(int OrderID)
        {
            try
            {
                using (SqlConnection con = new SqlConnection(ConnString))
                {
                    using (SqlCommand cmd = new SqlCommand("CheckCashMemoPrint", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@OrderID", OrderID);
                        cmd.CommandTimeout = 0;
        ...
        private bool CheckTokenPrintStatus(int OrderID)
        {
            try
            {
                using (SqlConnection con = new SqlConnection(ConnString))
                {
                    using (SqlCommand cmd = new SqlCommand("CheckInvicePrint", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@OrderID", OrderID);
                        cmd.CommandTimeout = 0;
        ...
        private void prnDocument_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            if (ReadInvoice == false)
            {
                ...
                AppConfiguration a = new AppConfiguration();
                UpdateTokenPrintStatus(a.OrderID);
                ReadInvoice = true;
        ...
        {
            AppConfiguration a = new AppConfiguration();
            bool Flag = CheckInvoicePrintStatus(a.OrderID);
            //if (Flag == true)
            //{
            //    for (int i = 0; i < 3; i++)
            //    {
