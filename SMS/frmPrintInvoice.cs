    ...
    public partial class frmPrintInvoice : Form
    {
        ...
        private void ReadInvoiceHead(int OrderID)
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("GetCompanyDetailsByOrderID", con))
                    {
        ...
        private int UpdateTokenPrintStatus(int OrderID)
        {
            try
            {
                using (SqlConnection con = new SqlConnection(ConnString))
                {
                    using (SqlCommand cmd = new SqlCommand("UpdateTaxInvoiceStatusByOrderID", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@OrderID", OrderID);
                        cmd.CommandTimeout = 0;
